...
import deckPic from "../pics/dekcPic.png";
...
export default function Decks() {
  const { idInventario, setIdMazo, setCarpetaNombre } =
  ...
  return (
    <div className="inventory__container">
          ...
          results.map((folder) => (
            <div className="userFolder">
              <img
                ...
                style={{ opacity: ".6" }}
                onClick={() => {
                  navigate("/Deck/Selected");
                  setIdMazo(folder.id_mazo);
                  setCarpetaNombre(folder.nombre);
                }}
              />
              <h3 style={{ color: "#1c4944" }}>{folder.nombre}</h3>
              <Cancel01Icon id="delete" />
              <PencilEdit02Icon
                id="edit"
                onClick={() => {
                  setEditVisible(true);
                  setFolderSelected(folder.id_mazo);
                }}
              />
            </div>
          ))
      ...
      <Create
        visible={visible}
        message={"Ingresa el nombre del mazo..."}
        setvisible={togleVisible}
        type={"mazo"}
      />
      <Edit
        visible={editVisible}
        message={"Ingresa el nuevo nombre del mazo..."}
